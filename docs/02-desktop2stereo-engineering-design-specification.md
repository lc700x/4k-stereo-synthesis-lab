...
9. OpenXR 异步重构以 `docs/35` / `docs/36` 为目标：虚拟显示器是硬实时路径，背景、Glow、墙面反射是软实时路径，不得阻塞 screen submit。
```
...
| OpenXR viewer | `src/xr_viewer/*.py` | OpenXR session、swapchain、D3D11/OpenGL render、controller/environment、runtime eye 上传；异步重构目标见 `docs/35` / `docs/36` |
...
| 步骤 11：Output Pack / Viewer Upload | `src/stereo_runtime/output.py`, `output_triton.py`, `src/viewer/*`, `src/xr_viewer/*`, `src/streaming/*` | 本地/网络消费 packed output，OpenXR 支持 full synthesis 与 RGB+depth direct；OpenXR/本地窗口共享 CUDA/GL texture uploader；runtime-eye tensor diagnostics 只能记录 shape/dtype/device 等 no-sync 信息；glow 实时采样必须走 GPU source texture，不允许回到 CPU tensor sampling；OpenXR async refactor 以 Quad-layer screen presenter / panorama background / async GPU Glow result pool 为目标；D3D11 direct parity 和 RTMP/低延迟 stream 是后续工程 |
...
`docs/35-OpenXR_Asynchronous_Decoupled_Rendering_Architecture_Report.md` 定义目标架构，`docs/36-OpenXR_Asynchronous_Decoupled_Rendering_Implementation_Plan.md` 定义实施计划。该目标高于当前局部实现状态，不能因为现有 Quad layer、Glow 或环境渲染路径尚未完全解耦而降低目标。

目标分层：

```text
Capture + StereoRuntime -> latest OpenXRRuntimeResult -> ScreenFrameBridge -> Quad layer screen presenter -> xrEndFrame
