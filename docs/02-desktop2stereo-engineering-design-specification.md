...
| 步骤 11：Output Pack / Viewer Upload | `src/stereo_runtime/output.py`, `output_triton.py`, `src/viewer/*`, `src/xr_viewer/*`, `src/streaming/*` | 本地/网络消费 packed output，OpenXR 支持 full synthesis 与 RGB+depth direct；OpenXR/本地窗口共享 CUDA/GL texture uploader；runtime-eye tensor diagnostics 只能记录 shape/dtype/device 等 no-sync 信息；glow 实时采样必须走 GPU source texture，不允许回到 CPU tensor sampling；D3D11 direct parity 和 RTMP/低延迟 stream 是后续工程 |
