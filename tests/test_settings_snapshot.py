...
def test_runtime_applies_hot_settings_snapshot_without_rebuild():
    ...
    assert runtime.active_settings_version == 7
...
def test_runtime_result_debug_info_tracks_active_settings_version():
    ...
    runtime.apply_settings_snapshot(RuntimeSettingsSnapshot(version=9, timestamp=1.0, depth_strength=1.2))

    result = runtime.process_openxr_frame(torch.zeros((1, 3, 2, 2), dtype=torch.float32))

    assert result.debug_info["active_settings_version"] == 9
...
def test_runtime_rejects_session_restart_snapshot():
    ...
    snapshot = RuntimeSettingsSnapshot(version=8, timestamp=1.0, device="cuda:1")
