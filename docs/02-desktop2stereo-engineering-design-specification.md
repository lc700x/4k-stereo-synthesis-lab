...
| OpenXR viewer | `src/xr_viewer/*.py` | OpenXR session、swapchain、Windows 默认 D3D11 native / OpenGL forced-or-auto fallback render、controller/environment、runtime eye 上传；主画面 3D 以 projection layer 为可靠路径，Quad layer 保留为实验/诊断/overlay |
...
3. OpenGL 作为本地 viewer 和兼容 fallback：继续验证 `GlTensorPboUploader` / `CudaGlTextureUploader` 的 image/PBO/CPU fallback 日志，不再把 OpenGL uploader 当作 D3D11 方案。
4. 移除 TensorRT ORT / ONNX Runtime depth provider 的 CPU numpy 往返，实现真正 GPU zero-copy input/output。
...
12. 继续 `docs/36` phase 2：证明 D3D11 native projection-layer 主画面在慢 runtime、慢 capture、慢/失败 effect worker 下复用 last-good frame，不阻塞 controller/head pose refresh、projection screen present 和 `xrEndFrame`。Quad layer 只保留为可选实验/诊断，不再作为完成判据。
